...
IMAGE_CMD_mbed-sdimg () {
	...
	"uImage")
		...
		mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}${KERNEL_INITRAMFS}-${MACHINE}.bin ::rescue_uImage
	...
	*)
	...
	then
		xzcat ${SDIMG_ROOTFS} | dd of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${RFS1START}) && sync && sync
		xzcat ${SDIMG_ROOTFS} | dd of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${RFS2START}) && sync && sync
	else
		#Add label to rootfs partition
		tune2fs -L rootfs1 ${SDIMG_ROOTFS}
		dd if=${SDIMG_ROOTFS} of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${RFS1START}) && sync && sync
		tune2fs -L rootfs2 ${SDIMG_ROOTFS}
		dd if=${SDIMG_ROOTFS} of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${RFS2START}) && sync && sync
		...
		dd if=${IMGDEPLOYDIR}/bootflags.ext3 of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${BOOTFLAGSSTART}) && sync && sync
		...
		dd if=${IMGDEPLOYDIR}/config.ext3 of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${CONFIGSTART}) && sync && sync
		...
		dd if=${IMGDEPLOYDIR}/cache.ext3 of=${SDIMG} conv=notrunc seek=1 bs=$(expr 1024 \* ${CACHESTART}) && sync && sync
	fi
